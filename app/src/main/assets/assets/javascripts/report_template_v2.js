...
(function() {
    ...
    window.ReportTemplateV2 = {
        ...
        generateTablesV2: function(outerIndex, innerIndex, tabs, partsindex) {
            ...
            htmlString = "<ul class='nav nav-tabs' id='nav-M' style='background-color:#2ec7c9;'>" +
                tmpArray.join("") +
                "</ul>";
            // clear array
            tmpArray.length = 0;
            ...
            ReportTemplateV2.tableIds[tabIndex] = $('#table_' + tabIndex).DataTable({
                ...
                createdRow: function(row, data, index) {
                    if (ReportTemplateV2.tables[tabIndex]['dataColumns'][data[0]] != undefined && ReportTemplateV2.tables[tabIndex]['dataColumns'][data[0]].length > 0) {
                        ...
                        $(row).eq(0).find('td').eq(0).html('<a href="javascript:void(0);" onclick="ReportTemplateV2.relatedTable(\'' + tabIndex + '\',\'' + $.trim(data[0]) + '\');"><img src="' + window.ReportTemplateV2.mobileAssetsPath('assets/images/bargtab_active_icon.png') + '"  /> ' + $(row).eq(0).find('td').eq(0).html() + '</a>')
                    }
                }
            });
        },
...
function main(selectedItem) {
    ...
    $(".dataTables_scrollBody").data('slt', { sl: this.scrollLeft, st: this.scrollTop }).scroll(function(e) {
        ...
        // 左右滚动
        console.log('左右滚动')
        $('body>table:first th:nth-child(1)').css({ width: '85px' })
        $('body>table:first th:nth-child(2)').css({ 'text-align': 'right' })
        $('body>table:first th:nth-child(2)').css({ 'textAlign': 'right' })
