...
public class NotificationService extends Service {
    ...
    private void showNotification(UMessage msg) {
        ...
        int reminder = 0;
        if (msg.play_sound) {
            reminder |= NotificationCompat.DEFAULT_SOUND;
        }
        if (msg.play_vibrate) {
            reminder |= NotificationCompat.DEFAULT_VIBRATE;
        }
        if (msg.play_lights) {
            reminder |= NotificationCompat.DEFAULT_LIGHTS;
        }
        mBuilder.setDefaults(reminder);
