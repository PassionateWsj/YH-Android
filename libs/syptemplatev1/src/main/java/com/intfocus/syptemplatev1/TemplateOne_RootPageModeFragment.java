 ...
 * 模块二根标签页面
 */
public class TemplateOne_RootPageModeFragment extends BaseModeFragment<MDetalRootPageMode> {
    ...
    public static TemplateOne_RootPageModeFragment newInstance(int suRootID, String param) {
        TemplateOne_RootPageModeFragment fragment = new TemplateOne_RootPageModeFragment();
    ...
    private void bindData(MDetalRootPageRequestResult result) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            switch (entity.type) {
                case "banner"://标题栏
                    fragment = TemplateOne_UnitBannerModeFragment.newInstance(entity.config);
                    break;

                case "chart"://曲线图表/柱状图(竖)
                    fragment = TemplateOne_UnitCurveChartModeFragment.newInstance(entity.config);
                    break;
                ...
                case "single_value"://单值组件
                    fragment = TemplateOne_UnitSingleValueModeFragment.newInstance(entity.config);
                    break;

                case "bargraph"://条状图(横)
                    fragment = TemplateOne_UnitPlusMinusChartModeFragment.newInstance(entity.config);
                    break;

                case "tables"://类Excel冻结横竖首列表格
                    fragment = TemplateOne_UnitTablesModeFragment.newInstance(suRootID, entity.config);
                    break;
            }
